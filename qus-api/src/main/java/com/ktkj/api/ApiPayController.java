...
import com.ktkj.entity.OrderVo;
import com.ktkj.entity.UserVo;
import com.ktkj.service.ApiOrderInfoService;
import com.ktkj.service.ApiOrderService;
import com.ktkj.util.ApiBaseAction;
import com.ktkj.util.wechat.WechatRefundApiResult;
import com.ktkj.util.wechat.WechatUtil;
import com.ktkj.utils.CharUtil;
import com.ktkj.utils.DateUtils;
import com.ktkj.utils.MapUtils;
import com.ktkj.utils.ResourceUtil;
import com.ktkj.utils.XmlUtil;
...
public class ApiPayController extends ApiBaseAction {
    ...
    @IgnoreAuth
    @PostMapping("prepay")
    public Object payPrepay(@RequestBody OrderInfoVo orderInfo) {

        if (null == orderInfo.getOrderId()) {
            return toResponsObject(400, "订单已取消", "");
        }

        OrderInfoVo orderInfoVo = apiOrderInfoService.queryById(orderInfo.getOrderId());

        if (null == orderInfoVo) {
            return toResponsObject(400, "订单已取消", "");
        }

        if ("1".equals(orderInfoVo.getOrderStatus())) {
        ...
        try {
            ...
            // 商户订单编号
            parame.put("out_trade_no", orderInfoVo.getOrderId());
            // 商品描述
            parame.put("body", "" + orderInfoVo.getGoodsName());
            //支付金额
            parame.put("total_fee", orderInfoVo.getTotalPayAmount());
            // 回调地址
            ...
            parame.put("openid", orderInfo.getOpenId());
...
//            Map<String, Object> resultUn = XmlUtil.xmlStrToMap(callbakcXml(orderInfo.getOrderId()));
            // 响应报文
            ...
            if (return_code.equalsIgnoreCase("FAIL")) {
                ...
                if (result_code.equalsIgnoreCase("FAIL")) {
                    ...
                    // 业务处理
                    orderInfoVo.setUniformorder(MapUtils.mapToString(resultUn));
                    apiOrderInfoService.update(orderInfoVo);
...
//            String reponseXml = callbakcXml("2019090345452509");
            ...
            if (result_code.equalsIgnoreCase("FAIL")) {
                ...
                // 业务处理
                OrderInfoVo orderInfoVo = apiOrderInfoService.queryById(out_trade_no);
                orderInfoVo.setOrderStatus("1");
                orderInfoVo.setPayTime(new Date());
                orderInfoVo.setNotify(result.toString());
                apiOrderInfoService.update(orderInfoVo);
                response.getWriter().write(setXml("SUCCESS", "OK"));
            }
        } catch (Exception e) {
